...
const CARD_TITLE = "Bugün Hafta Tutar"
const TOTAL_PREFIX = "Bugün hafta tutar:"

const WeekPaymenTokens = ({ productList }: Props) => {
  const weekPayment = useMemo(() => {
    return productList
      .filter((product) => {
        const date =
  ...
  return (
    <Card x-chunk="dashboard-01-chunk-0" className="w-full lg:w-1/3">
      ...
      <CardContent>
        <div className="text-2xl font-bold">+{formatCurrency(weekPayment)}</div>
        <p className="text-xs text-muted-foreground">
          {`${TOTAL_PREFIX} ${formatCurrency(weekPayment)}`}
        </p>
      </CardContent>
    </Card>
  )
...
export default memo(WeekPaymenTokens)
